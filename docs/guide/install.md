...
使用 `yarn` 安装 `vuepress-plugin-template-code` 组件：

```bash
yarn add vuepress-plugin-template-code -D
```

或者使用 `npm` 安装它：

```bash
npm i vuepress-plugin-template-code --save-dev
```
...
module.exports = {
  ...
  plugins: [[
    require('vuepress-plugin-template-code'),
    {
